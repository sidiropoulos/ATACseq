...
.fastqc:
	module load java/1.8.0 && cd src && wget 'http://www.bioinformatics.babraham.ac.uk/projects/fastqc/fastqc_v0.11.5.zip' && unzip fastqc_v0.11.5.zip && chmod 755 FastQC/fastqc && rm fastqc_v0.11.5.zip && cd ../ && touch .fastqc
...
.picard:
	module load java/1.8.0 && mkdir -p src/picard/ && cd src/picard && wget -O picard.jar 'https://github.com/broadinstitute/picard/releases/download/2.8.3/picard.jar' && cd ../../ && touch .picard

.freebayes:
	module load htslib/1.5 cmake/3.8.1 && cd src/freebayes && make && cd ../../ && touch .freebayes
